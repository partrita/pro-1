...
from pro1_critic import run_inference
...
def propose_mutations(sequence: str, enzyme_data: Dict, original_stability: float) -> str:
...
COPY THE FINAL SEQUENCE IN THE BRACKETS OF \\boxed{{}} TO ENCLOSE THE SEQUENCE. YOU MUST FOLLOW THIS INSTRUCTION/FORMAT. EX; \\boxed{{MALWMTLLLLPVPDGPK...}}"""
    ...
    response = run_inference(sequence=sequence, enzyme_data=enzyme_data, model=model, tokenizer=tokenizer, stability_calculator=stability_calculator, max_iterations=10, original_stability=original_stability)
    return response
...
def main():
    ...
    for enzyme_id, data in tqdm(selected_enzymes, desc="Processing enzymes"):
        ...
        try:
            ...
            # Get model response
            response = propose_mutations(sequence, enzyme_data, original_stability)
